...
public class LCPMessage {
	...
	public static final byte TOUCH = (byte) 0x01;
	...
	public static final byte SONAR_METRIC=(byte)0x0C;//not setable, just for return messages.
	public static final byte SONAR_INCH=(byte)0x0D;//not setable, just for return messages.
	public static final byte COMPASS=(byte)0x0E;//not setable, just for return messages.
	public static final byte IO_8574_SENSOR=(byte)0x0F;//not setable, just for return messages.
	public static final byte ADC_8591_SENSOR=(byte)0x10;//not setable, just for return messages.
	public static final byte COLOR=(byte)0x11;//not setable, just for return messages.
	public static final byte GYRO=(byte)0x12;//not setable, just for return messages.
	public static final byte TILT=(byte)0x13;//not setable, just for return messages.
	//End sensor Types. 
	...
	public static final byte ULTRASONIC_SENSOR =(byte) 0x0B;//custom, touch = 0x01
	//public static final byte REFLECTION = 0x0;
	public static final byte ULTRASONIC_MODE = (byte)0x20;
	public static final byte SIIIIIIII = (byte)0x42;
	//public static final byte SONAR_METRIC = 0x20;//test
	...
	//init the sensor with the specific port. 
	//pasar el sensor i el puerto luego. 
	public static byte[] getConnectionSensor(int sensor,byte type,byte mode) {
		byte[] message = new byte[12];
		//we wanna get reply to know if it works, should appear 0.
		message[0] = DIRECT_COMMAND_NOREPLY ;
		//type of message
		message[1] = SET_INPUT_MODE ;
		// Output port
		message[2] = (byte) sensor;
		//type of sensor we're gonna use
		message[3] = type;
		//type of data that sensor will return. 
		message[4] = mode;
	...
	//this inits the measurements of the ultrasonic sensor, now we init just the first(0). 
	public static byte[] getInitUltraSonicSensor(int sensor) {
		byte[] message = new byte[12];
		//byte[] message = {0x00, 0x0f, (byte)sensor, 0x03, 0x00, 0x02, 0x41, 0x04};
		...
		message[2] = (byte) sensor;
		//message[3] = 0x02;
		message[3] = PLAY_SOUND_FILE;
		//message[4] = 0x01;
		message[4] = STOP_PROGRAM;
		//message[5] = 0x02;
		message[5] = PLAY_SOUND_FILE;
		//message[6] = 0x42;
		message[6] = SIIIIIIII;//first measucrement 0

		return message;

	}

	//this gets the measurement value STATUS for the requested sensor in the mode that has been defined before. 
	public static byte[] checkMeasurementValue(int sensor) {
		byte[] message = new byte[12];
	...
	//set continous monitoring for the sensor. 
	public static byte[] setDefault(int sensor) {
		byte[] message = new byte[12];
	...
	//this gets the measurement value for the requested sensor in the mode that has been defined before. 
	public static byte[] getMeasurementValue(int sensor) {
		byte[] message = new byte[12];
	...
	public static byte[] getInputStateMessage(int port) {
		byte[] message = new byte[3];
