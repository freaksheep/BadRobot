...
public class UltraSonicSensor extends Thread {
	...
	public UltraSonicSensor(Handler btCommHandler, int port) {
		super();
	...
	public UltraSonicSensor(Handler btCommHandler, int port, boolean connected) {
		super();
		this.btCommHandler = btCommHandler;
		this.port = port;
	...
	public void run() {
		...
		while (connected ) {
			...
			try {
				Thread.sleep(500);
				sendBTCmessage(BTCommunicator.NO_DELAY, BTCommunicator.ULTRASONIC_SET, port,0);
				Thread.sleep(1000);
				sendBTCmessage(BTCommunicator.NO_DELAY, BTCommunicator.ULTRASONIC_GET, port,0);
			} catch (Exception e) {
