...
public class BTCommunicator extends Thread {
	...
	//pau test
	public static final int ULTRASONIC_ACTIVATION=10101;
	public static final int ULTRASONIC_SET=10102;
	public static final int ULTRASONIC_CHECK = 10103;
	public static final int ULTRASONIC_GET = 10104;
	public static final int NO_DELAY = 0;
	
	public static final int TOUCH_CONNECTION=10201;
	public static final int TOUCH_STATUS=10202;
	
	public static final int LIGHT_ON=10301;
	public static final int LIGHT_OFF=10302;
	...
	public BTCommunicator( Handler uiHandler, BluetoothAdapter btAdapter, Resources resources) {
	...
	private void dispatchMessage(byte[] message) {
		...
		case LCPMessage.GET_OUTPUT_STATE:
			...
			break;
			//ultrasonic test
		case LCPMessage.SET_INPUT_MODE:

			if (message.length >= 3)
				sendState(ULTRASONIC_ACTIVATION);

			break;
			
		case LCPMessage.LS_WRITE:

			if (message.length >= 3)
				sendState(ULTRASONIC_SET);
		...
		case LCPMessage.LS_GET_STATUS:
			...
			if (message.length >= 3)
				sendState(ULTRASONIC_CHECK);

			break;
		case LCPMessage.LS_READ:

			if (message.length >= 3)
				sendState(ULTRASONIC_GET);
	...
	//light functions
	private void turnOnLightSensor(int port){
		byte[] message = LCPMessage.getConnectionSensor(port ,LCPMessage.LIGHT_ACTIVE,LCPMessage.PERCENT_MODE);
		sendMessageAndState(message);
		
	}
	private void turnOffLightSensor(int port){
		byte[] message = LCPMessage.getConnectionSensor(port ,LCPMessage.LIGHT_INACTIVE,LCPMessage.PERCENT_MODE);
		sendMessageAndState(message);
		
	}
	
	//touch sensor functions
	private void initTouchSensor(int port){
		byte[] message = LCPMessage.getConnectionSensor(port ,LCPMessage.TOUCH,LCPMessage.BOOL_MODE);
		sendMessageAndState(message);
		
	}
	private void getTouchStatus(int port){
		byte[] message = LCPMessage.getInputStateMessage(port);
		sendMessageAndState(message);
		
	}
	//ultrasonic functions
	private void initReflectionSensor(){
		byte[] message = LCPMessage.getConnectionSensor(PORT_2 ,LCPMessage.LOWSPEED_9V,LCPMessage.SONAR_METRIC);
		sendMessageAndState(message);
		
		
	}
	//at the moment the set measurement just measures the first value(0).
	private void setMeasurement (){
		byte[] message= LCPMessage.getInitUltraSonicSensor(1);
		sendMessageAndState(message);
		try {
			//Thread.sleep(750);
		} catch (Exception e) {
	...
	//this asks for the sensor to get how is it's status. 
	private void checkMeasurement (){
		byte[] message= LCPMessage.checkMeasurementValue(1);
		sendMessageAndState(message);
	}
	//this asks for the sensor to get values. 
		private void getMeasurement (){
			byte[] message= LCPMessage.getMeasurementValue(1);
			sendMessageAndState(LCPMessage.setDefault(1));
			sendMessageAndState(message);
		}
	...
	final Handler myHandler = new Handler() {
		...
		public void handleMessage(Message myMessage) {
			...
			case MOTOR_C:
				...
				//Ultrasonic test
			case ULTRASONIC_ACTIVATION:
				initReflectionSensor();
				break;
			case ULTRASONIC_SET:
				setMeasurement();
			...
			case ULTRASONIC_CHECK:
				checkMeasurement();
				break;
			case ULTRASONIC_GET:
				Log.d("BLUETOOTH","btchandler distance get");
				getMeasurement();
				break;
				//end ultrasonic test
			...
			case TOUCH_CONNECTION:
				initTouchSensor(myMessage.getData().getInt("value1"));
				break;
				
			case TOUCH_STATUS:
				
				getTouchStatus(myMessage.getData().getInt("value1"));
				break;
				...
				//light test
			case LIGHT_ON:
				Log.d("BLUETOOTH","btchandler light on");
				turnOnLightSensor(myMessage.getData().getInt("value1"));
				break;
			case LIGHT_OFF:
				Log.d("BLUETOOTH","btchandler light off");
				turnOffLightSensor(myMessage.getData().getInt("value1"));
				break;
				//end light test
			case MOTOR_B_ACTION:
